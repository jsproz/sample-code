...
extension Riscv32 {
    ...
    public class Uret: Instruction {
        
        public init?() {
            super.init( opcode: .uret, size: 4 )
        }
        
        public func encode() -> UInt32 {
            let encoded: UInt32 = 0x200073
            return encoded
        }
    }
    ...
    public class Fence: Instruction {
        public let pred: fencearg
        public let succ: fencearg
        
        public init?( pred: UInt32, succ: UInt32 ) {
            guard let predDecoded = Riscv32.fencearg.decode( pred ) else { return nil }
            self.pred = predDecoded
            guard let succDecoded = Riscv32.fencearg.decode( succ ) else { return nil }
            self.succ = succDecoded
            super.init( opcode: .fence, size: 4 )
        }
        
        public func encode() -> UInt32 {
            var encoded: UInt32 = 0xf
            encoded |= ( pred.encode() & 0xf ) << 24
            encoded |= ( succ.encode() & 0xf ) << 20
            return encoded
        }
    }
    ...
    public class CBeqz: Instruction {
        ...
        public func encode() -> UInt32 {
            ...
            encoded |= ( imm.encode() & 0x100 ) << 4
            encoded |= ( imm.encode() & 0xc0 ) >> 1
            encoded |= ( imm.encode() & 0x20 ) >> 3
            encoded |= ( imm.encode() & 0x18 ) << 7
            encoded |= ( imm.encode() & 0x6 ) << 2
            encoded |= ( rs1.encode() & 0x7 ) << 7
            return encoded
        }
    }
    ...
    public class CJ: Instruction {
        ...
        public func encode() -> UInt32 {
            ...
            encoded |= ( offset.encode() & 0x800 ) << 1
            encoded |= ( offset.encode() & 0x400 ) >> 2
            encoded |= ( offset.encode() & 0x300 ) << 1
            encoded |= ( offset.encode() & 0x80 ) >> 1
            encoded |= ( offset.encode() & 0x40 ) << 1
            encoded |= ( offset.encode() & 0x20 ) >> 3
            encoded |= ( offset.encode() & 0x10 ) << 7
            encoded |= ( offset.encode() & 0xe ) << 2
            return encoded
        }
    }
    
    public class CJal: Instruction {
        public let offset: simm12_lsb0
        
        public init?( offset: UInt32 ) {
            guard let offsetDecoded = Riscv32.simm12_lsb0.decode( offset ) else { return nil }
            self.offset = offsetDecoded
            super.init( opcode: .cJal, size: 2 )
        }
        
        public func encode() -> UInt32 {
            var encoded: UInt32 = 0x2001
            encoded |= ( offset.encode() & 0x800 ) << 1
            encoded |= ( offset.encode() & 0x400 ) >> 2
            encoded |= ( offset.encode() & 0x300 ) << 1
            encoded |= ( offset.encode() & 0x80 ) >> 1
    ...
    public class CLui: Instruction {
        ...
        public func encode() -> UInt32 {
            ...
            encoded |= ( imm.encode() & 0x20000 ) >> 5
            encoded |= ( imm.encode() & 0x1f000 ) >> 10
            encoded |= ( rd.encode() & 0x1f ) << 7
            return encoded
        }
    }
    ...
    public class CSwsp: Instruction {
        ...
        public init?( imm: UInt32, rs2: UInt32 ) {
            guard let immDecoded = Riscv32.uimm8_lsb00.decode( imm ) else { return nil }
            self.imm = immDecoded
            guard let rs2Decoded = Riscv32.GPR.decode( rs2 ) else { return nil }
            self.rs2 = rs2Decoded
    ...
    public class Beq: Instruction {
        ...
        public func encode() -> UInt32 {
            ...
            encoded |= ( imm12.encode() & 0x1000 ) << 19
            encoded |= ( imm12.encode() & 0x800 ) >> 4
            encoded |= ( imm12.encode() & 0x7e0 ) << 20
            encoded |= ( imm12.encode() & 0x1e ) << 7
            encoded |= ( rs1.encode() & 0x1f ) << 15
            encoded |= ( rs2.encode() & 0x1f ) << 20
            return encoded
        }
    ...
    public class Auipc: Instruction {
        ...
        public func encode() -> UInt32 {
            ...
            encoded |= imm20.encode() & 0xfffff000
            encoded |= ( rd.encode() & 0x1f ) << 7
            return encoded
        }
    }
